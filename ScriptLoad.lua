...
local MY_HWID = "999999"  -- HWID cố định
...
do	
	...
	Credit.Text = [[<font color="#f8042e">Tele</font> @LuanOri04 | <font color="#5d6af2">DISCORD</font> .gg/LuanOri]]
	if IS_CUSTOM_UI_MODE then
        local yt = '<font color="#f8042e">YT</font> @' .. (CUSTOM_UI_MODE_DATA.YouTube or "LuanOri04")
        local dc = '<font color="#5d6af2">DISCORD</font> .gg/' .. (CUSTOM_UI_MODE_DATA.Discord or "LuanOri")
        Credit.Text = yt .. " | " .. dc
    end
...
function api.check_key(key)
    -- Dùng HWID cố định thay vì tick()
    local url = API_URL .. "/api/check_key?key=" .. key .. "&hwid=" .. MY_HWID
...
function api.load_script()
    ...
    if status.data and status.data.new_key then
        ...
        writefile("LuanOriKeyV4.txt", key)
    ...
    local link = API_URL .. '/api/getkey?hwid=' .. MY_HWID
    if IS_CUSTOM_UI_MODE and CUSTOM_UI_MODE_DATA.GetKeyLink then
    ...
    setclipboard(IS_CUSTOM_UI_MODE and CUSTOM_UI_MODE_DATA.TutorialLink or API_URL .. '/api/getkey?hwid=TUTORIAL')
    StarterGui:SetCore("SendNotification",{
        Title = "Key System",
	...
	do
		if isfile("LuanOriKeyV4.txt") or _G.MY_KEY_IS then
			local key = _G.MY_KEY_IS and _G.MY_KEY_IS or readfile("LuanOriKeyV4.txt")
            Frame.Textbox.Text = key
			do_check_key(key)
		end
	end
